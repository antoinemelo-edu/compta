# Antoine MELO :: COMPTA :: v0.7 :: 17 février 2024

import tkinter, os, sys, shutil, webbrowser, re, unicodedata, requests
from tkinter import Text, ttk, messagebox, scrolledtext, simpledialog
...
def lancer_interface(pc_tri, si):
    global f_soldes
    try:
        if 'f_soldes' in globals():
            if f_soldes.winfo_exists():
                f_soldes.destroy()
    except tk.TclError:
        pass
    f_soldes = tk.Tk()
...
def journaliser_entree_facture(entry_number):
    journal_facture = tk.Tk()
    journal_facture.withdraw()

    entree_date = None
    clic_cancel = verif = 0
    while entree_date is None or len(entree_date) != 8:
        entree_date = simpledialog.askstring("Détails / Date", "Date de la facture (YYYYmmdd):", parent=journal_facture)
        if entree_date is None:  # L'utilisateur a cliqué sur Cancel
            clic_cancel = verif = 1
            break  # Sortir de la boucle et ne pas procéder plus loin

    entree_delai = simpledialog.askstring("Détails / Délai", "Délai de la facture (en jours):", parent=journal_facture)

    if entree_date is None or len(entree_date) == 0:
        date_aujourdhui = datetime.now()
        entree_date = date_aujourdhui.strftime('%Y%m%d')

    if entree_delai is None or len(entree_delai) == 0:
        clic_cancel = 2
        entree_delai = 0

    numero_modifie = f"0{entry_number}" if entry_number < 10 else entry_number
    nom_fichier = f"{entree_date}_{numero_modifie}_{entree_delai}"
    chemin_factures = f"{dossier_mandat}/factures" 
    chemin_facture = f"{chemin_factures}/{nom_fichier}"

    if clic_cancel == 0:
        os.makedirs(os.path.dirname(chemin_facture), exist_ok=True)
        with open(chemin_facture, 'a', encoding='utf-8') as file:
            file.write("")

    if clic_cancel == 1 and len(entree_delai) > 0:
        os.makedirs(os.path.dirname(chemin_facture), exist_ok=True)
        with open(chemin_facture, 'a', encoding='utf-8') as file:
            file.write("")
        messagebox.showinfo("Message", "La facture s'enregistrera avec la date d'aujourd'hui.")

    if clic_cancel == 2 and verif == 0:
        os.makedirs(os.path.dirname(chemin_facture), exist_ok=True)
        with open(chemin_facture, 'a', encoding='utf-8') as file:
            file.write("")
        messagebox.showinfo("Message", "La facture s'enregistrera sans délai de paiement (immédiat).")

    if clic_cancel == 2 and verif == 1:
        messagebox.showinfo("Message", "Aucun suivi de facture ne sera visible pour cette écriture.")

    journal_facture.destroy()
...
def ajouter_entree(journal_path, debit_account, credit_account, label, amount_entry, comptes_dict):
    ...
    if isinstance(debit_account, str) or isinstance(credit_account, str):
        # Construire la chaîne complète pour le compte de débit
        if debit_account in comptes_dict:
            debit_selection = f"{debit_account} - {comptes_dict[debit_account]}"
        else:
            messagebox.showerror("Erreur", f"Le compte de débit {debit_account} n'existe pas dans le plan comptable.")
            return
        
        # Construire la chaîne complète pour le compte de crédit
        if credit_account in comptes_dict:
            credit_selection = f"{credit_account} - {comptes_dict[credit_account]}"
        else:
            messagebox.showerror("Erreur", f"Le compte de crédit {credit_account} n'existe pas dans le plan comptable.")
            return
    else:
    ...
    if isinstance(label, str):
        ...
        full_label = label.get()

    if not isinstance(amount_entry, float):
        try:
    ...
    deux_caracteres_debit = debit_selection[:2]
    deux_caracteres_credit = credit_selection[:2]
    comptes_facture = modes_facture_tuple(f"{dossier_mandat}/facture.txt")

    type_operation = ""
    if deux_caracteres_debit in comptes_facture and deux_caracteres_debit.startswith('1'):
        type_operation = "debit_ok"
    if deux_caracteres_credit in comptes_facture and deux_caracteres_credit.startswith('2'):
        type_operation = "credit_ok"

    if deux_caracteres_credit in comptes_facture and deux_caracteres_credit.startswith('1'):
        type_operation = "credit_no"
        nom_compte_no = credit_selection
    if deux_caracteres_debit in comptes_facture and deux_caracteres_debit.startswith('2'):
        type_operation = "debit_no"
        nom_compte_no = debit_selection
        
    if type_operation == "debit_ok" or type_operation == "credit_ok":
        journaliser_entree_facture(entry_number)

    if type_operation == "debit_no" or type_operation == "credit_no":
        messagebox.showinfo("Message", f"Passer par \n\nExtrait de compte \n{nom_compte_no} \n\npour enregistrer les paiements de factures.")
    else:
        with open(journal_path, 'a', encoding='utf-8') as file:
            file.write(f"{entry_number}::{debit_numero}::{credit_numero}::{full_label}::{amount}\n")
    ...
    if isinstance(debit_account, ttk.Combobox) or isinstance(credit_account, ttk.Combobox):
...
def importer_transactions(delete_url, url, journal_path, comptes_dict):
    try:
        response = requests.get(url)
        response.raise_for_status()  # Lève une exception pour les réponses non satisfaisantes
        
        # Diviser le contenu du fichier en lignes
        lignes = response.text.strip().splitlines()
        ...
        for ligne in lignes:
            ...
            date, debit_account, credit_account, label, amount_entry = ligne.split("::")
            # Convertir amount_entry en float
            try:
                amount_entry = float(amount_entry)
            except ValueError:
                print(f"Montant invalide pour la ligne : {ligne}")
                continue  # Passer à la ligne suivante si le montant n'est pas un nombre valide
            
            ajouter_entree(journal_path, debit_account, credit_account, label, amount_entry, comptes_dict)
        
        messagebox.showinfo("Importation", "Importation terminée avec succès. Bien vérifier que le fichier des transactions a été correctement effacé.")

        webbrowser.open(delete_url)
...
def lancer_interface_ajout_entree(journal_path, comptes_dict):
    ...
    global f_journaliser, label_url
    try:
        if 'f_journaliser' in globals():
            if f_journaliser.winfo_exists():
                f_journaliser.destroy()
    except tk.TclError:
        pass
    f_journaliser = tk.Tk()
    f_journaliser.title("Ajouter une écriture à double entrée au journal")
    ...
    if os.path.exists("config/config.txt"):
        ...
        entreprise_modifiee = entreprise.replace("/", "_").replace("\\", "_")
        nom_modifiee = nom.replace("/", "_").replace("\\", "_")
        entreprise_nettoyee = nettoyer_chaine(entreprise_modifiee)
        nom_nettoye = nettoyer_chaine(nom_modifiee)
        delete_url = f"https://parapente.academy/autre/compta/effacer.php?user={entreprise_nettoyee}&mandat={nom_nettoye}"
        url = f"https://parapente.academy/autre/compta/{entreprise_nettoyee}/{nom_nettoye}/transactions.txt"
        ...
        try:
            ...
            if response.status_code == 200 and len(response.content) > 0:
                ...
                nombre_de_lignes = len(response.text.splitlines())
                if nombre_de_lignes == 1:
                    texte_transac = "Journaliser la transaction enregistrée sur le web"
                else:
                    texte_transac = f"Journaliser les {nombre_de_lignes} transactions enregistrées sur le web"
                
                label_url = tk.Label(f_journaliser, text=texte_transac, fg="red", cursor="hand2")
                label_url.grid(row=2, column=0, columnspan=4, padx=10, pady=5)
                label_url.bind("<Button-1>", lambda e: importer_transactions(delete_url, url, journal_path, comptes_dict))
...
def ouvrir_listing_compte(compte_selectionne_entier):
    ...
    if any(compte_selectionne_entier.startswith(f"{numero} -") for numero in comptes_dict):
        ...
        global f_listing_compte
        try:
            if 'f_listing_compte' in globals():
                if f_listing_compte.winfo_exists():
                    f_listing_compte.destroy()
            if 'f_details_compte' in globals():
                if f_details_compte.winfo_exists():
                    f_details_compte.destroy()
        except tk.TclError:
            pass
        f_listing_compte = tk.Toplevel()
        f_listing_compte.title(f"Extrait de compte")

        listing_frame = tk.Frame(f_listing_compte)
        listing_frame.grid(row=1, column=0, sticky="ew")
        tk.Button(listing_frame, relief="flat", text="Sous forme de compte en T?", 
                  command=lambda: ouvrir_details_compte()).pack(pady=5)

        # Utiliser un Frame pour contenir le label et le Text widget
        content_frame = tk.Frame(f_listing_compte)
        ...
        f_listing_compte.grid_rowconfigure(0, weight=1)
        f_listing_compte.grid_columnconfigure(0, weight=1)
        ...
        if compte_selectionne.startswith('1'):
            lignes_debit.append(f"{' ':>2}Si) {str(solde_initial).rjust(10)}{' ':>2} | {' ':>5} ---")
            somme_debit += solde_initial
        elif compte_selectionne.startswith('2'):
            lignes_credit.append(f"{' ':>2}Si) {' ':>6} --- {' ':>1} | {str(solde_initial).rjust(9)}")
                ...
                for line in journal_file:
                    ...
                    if compte_selectionne in (debit, credit):
                        if compte_selectionne == debit:
                            lignes_debit.append(f"{no_piece.rjust(4)}) {str(montant).rjust(10)}{' ':>2} | {' ':>5} ---")
                            somme_debit += montant
                        else:
                            lignes_credit.append(f"{no_piece.rjust(4)}) {' ':>6} --- {' ':>1} | {str(montant).rjust(9)}")
        ...
        for ligne in lignes_debit:
            ecritures.append((ligne, "debit"))
        for ligne in lignes_credit:
            ecritures.append((ligne, "credit"))

        # Fonction pour déterminer la priorité de tri
        def cle_tri(ecriture):
            no_piece, type_ecriture = ecriture
            # Donner la priorité aux lignes commençant par "Si"
            if "Si)" in no_piece:
                return ("", type_ecriture)  # Les éléments "Si" seront triés en premier
            return (no_piece, type_ecriture)

        # Trier les écritures en utilisant la fonction de clé personnalisée
        ecritures.sort(key=cle_tri)

        # Afficher les écritures triées
        for ecriture, type_ecriture in ecritures:
            if type_ecriture == "debit":
                # Alignement pour le débit
                text_widget.insert(tk.END, f"{ecriture}\n")
            else:
                # Alignement pour le crédit, ajustez l'espacement selon vos besoins
                text_widget.insert(tk.END, f"{ecriture}\n")
        ...
        btn_frame = tk.Frame(f_listing_compte)
        btn_frame.grid(row=2, column=0, sticky="ew")
        tk.Button(btn_frame, text="Opération·s liée·s à ce compte", 
                  command=lambda: saisir_operations_compte(compte_selectionne_entier, f_listing_compte, dossier_mandat)).pack(pady=5)

        hauteur_fenetre = min(len(text_widget.get(1.0, 'end').split('\n'))*20+65, 400)
        largeur_fenetre = 280
        f_listing_compte.geometry(f"{largeur_fenetre}x{hauteur_fenetre}")
    else:
...
def ouvrir_details_compte():
    ...
    if any(compte_selectionne_entier.startswith(f"{numero} -") for numero in comptes_dict):
        ...
        f_details_compte.title(f"Compte en T")

        listing_frame = tk.Frame(f_details_compte)
        listing_frame.grid(row=1, column=0, sticky="ew")
        tk.Button(listing_frame, relief="flat", text="Voir cet extrait sous forme de liste", 
                  command=lambda: ouvrir_listing_compte(compte_selectionne_entier)).pack(pady=5)
...
def enregistrer_operation_compte(compte_principal, montant_debit_var, montant_credit_var, libelle_var, compte_contrepartie, facture, date_var, delai_var, piece_impact, paiement_var):
    ...
    if facture:
        ...
        elif len(piece_saisie) > 0 and paiement == 1:
            piece_saisie = int(piece_saisie)
            numero_modifie = f"0{piece_saisie}" if piece_saisie < 10 else piece_saisie
            texte_ref = f"Réf. # {piece_saisie}"
            nom_fichier = f"_{numero_modifie}_"            
...
def afficher_journal(journal_path):
    global f_journal
    try:
        if 'f_journal' in globals():
            if f_journal.winfo_exists():
                f_journal.destroy()
    except tk.TclError:
        pass
    f_journal = tk.Tk()
...
def afficher_grand_livre(accounts, initial_balances, account_totals, final_balances):
    global f_grandlivre
    try:
        if 'f_grandlivre' in globals():
            if f_grandlivre.winfo_exists():
                f_grandlivre.destroy()
    except tk.TclError:
        pass
    f_grandlivre = tk.Tk()
    f_grandlivre.title("Grand-Livre")

    text_widget = tk.Text(f_grandlivre, wrap='none', width=87)
    text_widget.pack(expand=True, fill='both')

    header = " {:<41} {:>10} {:>10} {:>10} {:>10}\n".format("Comptes du GL ", "Si  ", "Débit ", "Crédit ", "Sf/Vr ")
    text_widget.insert(tk.END, header)


    # Ajouter le soulignement après le header
    underline = " {:<41} {:>10} {:>10} {:>10} {:>10}\n".format("----------------------- ", "------", "--------", "--------", "-------")
    text_widget.insert(tk.END, underline)
    ...
    for account in sorted(final_balances.keys(), key=int):
        if initial_balances.get(account, 0) != 0 or account_totals[account]['debit'] != 0 or account_totals[account]['credit'] != 0 or final_balances[account] != 0:
            line = " {:<5} {:<35} {:>10.2f} {:>10.2f} {:>10.2f} {:>10.2f}\n".format(
                account,
                accounts[account][:35],
...
def nettoyer_chaine(chaine):
    # Normaliser la chaîne pour décomposer les lettres accentuées
    chaine_normalisee = unicodedata.normalize('NFKD', chaine)
    
    # Filtrer pour ne garder que les caractères alphanumériques
    sans_accents = ''.join(c for c in chaine_normalisee if unicodedata.category(c) != 'Mn')
    
    # Supprimer tous les caractères non alphanumériques
    return re.sub(r'\W+', '', sans_accents)

def ouvrir_url(url):
    webbrowser.open(url)
    
def afficher_texte():
    global fenetre_texte
    try:
        if 'fenetre_texte' in globals():
            if fenetre_texte.winfo_exists():
                fenetre_texte.destroy()
    except tk.TclError:
        pass
  ...
  - Backup lors d'effacement de la journalisation
  - Export / import des soldes finaux
  - Plus de cent écritures
  - Saisie de facture sans éritures comptables
  - Salaires
                                                 - - -
  Version βeta 0.8a du 23 février 2024
                                                 - - -"""
    ...
    # Ouvre le lien vers les transactions
    if os.path.exists("config/config.txt"):
        with open("config/config.txt", "r", encoding='utf-8') as fichier:
            for ligne in fichier:
                if ligne.startswith("Nom:"):
                    ...
                    entreprise = ligne.split("Entreprise:")[1].strip()
    if len(nom) > 4 and len(entreprise) > 4:
        entreprise_modifiee = entreprise.replace("/", "_").replace("\\", "_")
        nom_modifiee = nom.replace("/", "_").replace("\\", "_")
        entreprise_nettoyee = nettoyer_chaine(entreprise_modifiee)
        nom_nettoye = nettoyer_chaine(nom_modifiee)
        url = f"https://parapente.academy/autre/compta/etape1.html?user={entreprise_nettoyee}&mandat={nom_nettoye}"

        transac_a_afficher = """Ce logiciel est complété par un autre petit outil,
disponible sur le web, pour effectuer
l'enregistrement, à distance, de transactions
courantes."""
        label_transac = tk.Label(fenetre_texte, text=transac_a_afficher, justify=tk.LEFT)
        label_transac.pack()
    
        label_url = tk.Label(fenetre_texte, text="Lien vers le mandat en cours (web)", fg="blue", cursor="hand2")
        label_url.pack()
        label_url.bind("<Button-1>", lambda e: ouvrir_url(url))

        label_transacbis = tk.Label(fenetre_texte, text="- - -")
        label_transacbis.pack()
    ...
    if os.path.exists(chemin_pdf):
        ...
        label_pdf.pack()
        label_pdf.bind("<Button-2>", lambda e: ouvrir_pdf())
...
def ouvrir_fenetre_instructions(nom_mandat):
    ...
    tk.Label(fenetre_instructions, text="COMPTES POUR LE SUIVI DE FACTURES (expl*: 0 ou 11, 20, 22)", font=("Courier", 10, "bold")).grid(row=7, column=0, columnspan=2)
    tk.Label(fenetre_instructions, text="* 11 pour les créances, 20 pour les dettes fournisseurs, 22 pour les autres dettes", font=("Courier", 8, "italic")).grid(row=9, column=0, padx=15, columnspan=2, sticky='w')
...
def ouvrir_config(fenetre_principale):
    global fenetre_config
    try:
        if 'fenetre_config' in globals():
            if fenetre_config.winfo_exists():
                fenetre_config.destroy()
    except tk.TclError:
        pass
...
def afficher_exercice(nom_exercice):
    global fenetre_exercice
    try:
        if 'fenetre_exercice' in globals():
            if fenetre_exercice.winfo_exists():
                fenetre_exercice.destroy()
    except tk.TclError:
        pass
...
def afficher_totaux(totaux_mandat):
    global fenetre_totaux
    try:
        if 'fenetre_totaux' in globals():
            if fenetre_totaux.winfo_exists():
                fenetre_totaux.destroy()
    except tk.TclError:
        pass
...
def afficher_ecritures_journal(fenetre, rep_journal, dossier_factures, nom_compte, no_compte):
    ...
    for ecriture in ecritures_filtrees:
        ...
        dos = f"{dossier_mandat}/factures"
        nom_fichier = f"_{no}_"
        somme = 0
        for fichier in os.listdir(dos):
            chemin_complet = os.path.join(dos, fichier)
            with open(chemin_complet, 'r', encoding='utf-8') as fich:
                for ligne in fich:
                    try:
                        nombre = float(ligne.strip())  # strip() enlève les espaces/blancs au début et à la fin, y compris les sauts de ligne
                        somme += nombre
                    except ValueError:
                        print(f"La ligne '{ligne.strip()}' n'est pas un nombre valide et sera ignorée.")
        montant = float(montant) - somme
...
def saisir_operations_facture(type_utilise, moins_plus, compte_info, rep_mandat, paiement, no_pieces_facture):
    ...
    # Créances clients
    if compte_info_num.startswith('110') and type_utilise == "debit":
        no_filtre = ['3']
    # Autres créances
    elif (compte_info_num.startswith('111') or compte_info_num.startswith('112') or compte_info_num.startswith('113') or compte_info_num.startswith('114') or compte_info_num.startswith('115') or compte_info_num.startswith('116') or compte_info_num.startswith('117') or compte_info_num.startswith('118') or compte_info_num.startswith('119')) and type_utilise == "debit":
        no_filtre = ['1', '4', '5', '6', '7', '8']
    # Actifs
    ...
    # Passifs
        ...
        no_filtre = ['1']
    # Dettes fournisseurs
    elif compte_info_num.startswith('200') and type_utilise == "credit":
        no_filtre = ['4']
    # Autres dettes
    elif (compte_info_num.startswith('201') or compte_info_num.startswith('202') or compte_info_num.startswith('203') or compte_info_num.startswith('204') or compte_info_num.startswith('205') or compte_info_num.startswith('206') or compte_info_num.startswith('207') or compte_info_num.startswith('208') or compte_info_num.startswith('209') or compte_info_num.startswith('21') or compte_info_num.startswith('22')) and type_utilise == "credit":
        no_filtre = ['10', '14', '15', '16', '17', '18', '19', '5', '6', '7', '8']
    elif compte_info_num.startswith('1') and type_utilise == "":
        no_filtre = ['1', '2', '3', '4', '5', '6', '7', '8']
        type_utilise == "debit"
    ...
    if paiement:
        tk.Label(f_operations_facture, text="No de pièce comptable impacté").grid(row=8, column=0, sticky="w")
