# Antoine MELO :: COMPTA :: v0.5 :: 10 février 2024
...
def saisir_operations_compte(compte_info, fenetre_a_fermer, rep_mandat):
    ...
    if mode_facture:
        ...
        tk.Button(f_operations_compte, text="Ajouter", command=lambda: enregistrer_operation_compte(compte_info, montant_debit_var, montant_credit_var, libelle_var, compte_contrepartie.get(), False, date_var, delai_var, "", "")).grid(row=5, column=0, columnspan=2, sticky="ew")
...
def afficher_texte():
    ...
    ---
    Version βeta 0.5b du 10 février 2024
    ---"""
...
def saisir_operations_facture(type_utilise, moins_plus, compte_info, rep_mandat, paiement, no_pieces_facture):
    ...
    # Déterminer les signes à ajouter après les labels de débit et de crédit
    if premier_nombre in moins_plus:
        label_debit_text = "Montant au débit (- -)"
        label_credit_text = "Montant au crédit (++)"
    else:
        label_debit_text = "Montant au débit (++)"
        label_credit_text = "Montant au crédit (- -)"
