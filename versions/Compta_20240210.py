# Antoine MELO :: COMPTA :: v0.5 :: 9 février 2024
...
def ouvrir_details_compte():
    ...
    if any(compte_selectionne_entier.startswith(f"{numero} -") for numero in comptes_dict):
        ...
        tk.Button(btn_frame, text="Nouvelle·s opération·s liée·s à ce compte", 
                  command=lambda: saisir_operations_compte(compte_selectionne_entier, f_details_compte)).pack(pady=5)
...
def selection_compte(f_compte, comptes_dict):
    ...
    frame_selection_compte.grid(row=1, column=0, columnspan=2, sticky='w')
...
def saisir_operations_compte(compte_info, fenetre_a_fermer):
    ...
    if compte_numero.startswith('2') or compte_numero.startswith('3'):
        label_debit_text = "· débit (- -)"
        label_credit_text = "· crédit (++)"
    else:
        label_debit_text = "· débit (++)"
        label_credit_text = "· crédit (- -)"
    ...
    tk.Button(f_operations_compte, text="Ajouter", command=lambda: enregistrer_operation_compte(compte_info, montant_debit_var, montant_credit_var, libelle_var, compte_contrepartie.get())).grid(row=5, column=0, columnspan=2, sticky="ew")
...
def enregistrer_operation_compte(compte_principal, montant_debit_var, montant_credit_var, libelle_var, compte_contrepartie):
    ...
    def afficher_message_temporaire(message):
        ...
        message_label.grid(row=6, column=0, columnspan=2, sticky="ew")
...
def afficher_texte():
    ...
    ---
    Version βeta 0.5a du 9 février 2024
    ---"""
    fenetre_texte = tk.Toplevel()
    fenetre_texte.title("A propos")
    label = tk.Label(fenetre_texte, text=texte_a_afficher)
    label.pack()

# CONFIG #########################################################################
def vider_journal(journal_mandat):
    if len(journal_mandat) > 0:
        open(f"config/{journal_mandat}/journal.txt", "w").close()
    else:
        open(f"config/journal.txt", "w").close()
...
def ouvrir_fenetre_instructions(nom_mandat):
    ...
    tk.Label(fenetre_instructions, text="CONSIGNES", font=("Courier", 10, "bold")).grid(row=0, column=0, pady=5, columnspan=2)
    text_instructions = scrolledtext.ScrolledText(fenetre_instructions, height=20, width=80)
    ...
    tk.Label(fenetre_instructions, text="------------------------------------------------------------------", font=("Courier")).grid(row=5, column=0, columnspan=2)
    tk.Label(fenetre_instructions, text="AFFICHAGE BILAN ET COMPTES DE RÉSULTAT", font=("Courier", 10, "bold")).grid(row=6, column=0, columnspan=2)
    text_totaux = scrolledtext.ScrolledText(fenetre_instructions, height=25, width=80)
    text_totaux.grid(row=7, column=0, columnspan=2, padx=10, pady=5)
    charger_contenu(text_totaux, f"config/{nom_mandat}/totaux.txt")

    bouton_sauvegarder = tk.Button(fenetre_instructions, text="Sauvegarder", command=lambda: sauvegarder_modifications(fenetre_instructions, text_instructions, text_moinsplus, text_totaux, rep_mandat))
    bouton_sauvegarder.grid(row=8, column=0, columnspan=2, pady=5)

def sauvegarder_modifications(fenetre_a_fermer, text_instructions, text_moinsplus, text_totaux, rep_mandat):
...
def afficher_exercice(nom_exercice):
    ...
    if os.path.exists(chemin_exercice):
        ...
        fenetre_exercice.title(f"Consigne·s  ::  {nom_titre}")
        ...
        with open(chemin_exercice, "r", encoding="utf-8") as fichier:
            contenu = fichier.read()
            texte_exercice.insert(tk.END, contenu)
        texte_exercice.config(state=tk.DISABLED)

def verifier_exercice(fenetre, repertoire_exercice, nom_exercice):
    global bouton_exercice  # Rendre le bouton accessible globalement pour pouvoir le cacher/réafficher

    rep_fichier = f"{repertoire_exercice}/exercice.txt"
    if os.path.exists(rep_fichier) and os.path.getsize(rep_fichier) > 0:
        if 'bouton_exercice' not in globals():  # Créer le bouton s'il n'existe pas
            bouton_exercice = tk.Button(fenetre, relief="flat", text="Consigne·s", fg="blue", command=lambda: afficher_exercice(nom_exercice))
            bouton_exercice.grid(row=4, column=0, padx=10, pady=5, sticky='w')
        else:
            bouton_exercice.grid()  # Réafficher le bouton s'il était précédemment caché
    else:
        if 'bouton_exercice' in globals():
            bouton_exercice.grid_remove()  # Cacher le bouton
...
if not os.path.exists(f"config/{base_config}"):
    open(f"config/{base_config}", 'w').close()
...
chemin_mandat = os.path.join(os.getcwd(), f"config/{nom_mandat}")
...
if nom_mandat == "O":
    ...
    dossier_mandat = f"config/{nom_mandat}"
...
bouton_totaux.grid(row=3, column=0, columnspan=2, padx=10, pady=5, sticky='ew')

bouton_about = tk.Button(fenetre, relief="flat", text="i", command=afficher_texte)
bouton_about.grid(row=4, column=1, padx=10, pady=5, sticky='w')

#chemin_repertoire = tk.Label(fenetre, text = " " + os.path.dirname(os.path.realpath(__file__)), font=('Courier', 8))
#chemin_repertoire.grid(row=4, column=0, columnspan=2)
